...
class WindContractorPath(MonteCarlo.Path):
    ...
    def calculate_cash_flows(self, 
        ...
        CAPEX_WC, OPEX,
        r, 
        t_construction, LT_field, LT_turbine,
        wind_annual_power_production, wind_residual_ppa, 
        CAPEX_support,
        **kwargs
    ):
        ...
        wind_residual_value = (
            (
                wind_annual_power_production*wind_residual_ppa # Value assuming PPA-price of 1200
                - OPEX
            ) * sum(
                [
                    1/(1+r)**i 
                    for i in range(
                        LT_turbine-LT_field+t_construction
                    )
                ]
            )
        ) 
        ...
        return net_present_value(self.cash_flows, r)
...
class FieldOperatorPath(MonteCarlo.Path):
    ...
    def calculate_cash_flows(self,
        ...
        gas_price_conversion_factor, carbon_price_conversion_factor,
        carbon_price_ceiling,
        **kwargs
    ):
        ...
        # Carbon costs
        P_ets = np.minimum(self.state_variables["P_ets"], carbon_price_ceiling) * carbon_price_conversion_factor # converted to NOK / tCO2-eq
        ...
        # Added gas sales
        P_gas_t = self.state_variables["P_gas"] * gas_price_conversion_factor # converted to NOK / Sm^3 
        ...
        # NOx fund
        self.government_funding_t = np.array(
            [0 for i in range(t_construction)] +
            [(
